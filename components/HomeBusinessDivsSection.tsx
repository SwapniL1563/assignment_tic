...
export default function BusiDivSection(){
   ...
   return (
    <section className="w-full py-10 md:py-16 px-6 md:px-20 cursor-default">
      ...
      <div className="grid md:grid-cols-3 gap-6">
        <div className="relative rounded-2xl overflow-hidden md:col-span-2 h-60 md:h-80">
          <Image
            src={divisions[0].img}
        ...
        {divisions.slice(1).map((item, idx) => (
          <div
            key={idx}
            className="relative rounded-2xl overflow-hidden h-60 md:h-80"
          >
            <Image
              src={item.img}
              alt={item.title}
